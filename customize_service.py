...
def get_model(model_path, **kwargs):
    ...
    exp = get_exp("exps/example/yolo_mango.py", "yolox_x")
    model = exp.get_model()
    logger.info(
...
class PTVisionService(PTServingBaseService):
    ...
    def _inference(self, data):
        ...
        origin_shape = (image1.shape[1],image1.shape[0]) # C H W

        outputs, img_info=self.model.inferenceImg(image1)
        result['detection_classes'] = []
        result['detection_scores'] = []
        result['detection_boxes'] = []
        outputs=outputs[0].numpy()[0]
        b = [outputs[1],outputs[0], outputs[3], outputs[2]]  # y1 x1 y2 x2
        result['detection_classes'].append(self.class_map[int(outputs[6])])

        result['detection_scores'].append(round(outputs[4], 5))
